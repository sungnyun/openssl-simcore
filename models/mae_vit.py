...
def _get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    pos = pos.reshape(-1)  # (M,)  M == H*W (grid_size * grid_size)
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def interpolate_pos_embed(self):
        pos_embed_checkpoint = self.pos_embed
    ...
    def forward(self, imgs, pretrain=False, global_pool=False):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio=self.mask_ratio)
        if not pretrain:  # for linear evaluation
            if not global_pool:
                # we don't use 'global_pool' for the evaluation
                # latent = latent[:, 1:, :].mean(dim=1)  # global pool without cls token
                # return self.fc_norm(latent)  # self.fc_norm = norm_layer(embed_dim)
                return latent[:, 0, :]
            else:
                # this case is only for train_selfsup_sampling.py
